class Day11(test: Boolean = false): Day(test, 1656, 0) {
    ...
    private fun findNeighbors(position: Pair<Int, Int>,
                              flashed: Set<Pair<Int, Int>>,
                              positionMap: MutableMap<Pair<Int, Int>, Int>): List<Pair<Int, Int>> {
        val (first, second) = position
        return setOf(Pair(first-1, second), Pair(first+1, second),
